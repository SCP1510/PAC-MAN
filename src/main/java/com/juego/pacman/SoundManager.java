...
public class SoundManager {
    ...
    static {
        ...
        gameplayPlayer =
                load("/assets/sounds/17. Game Play.mp3");

        startPlayer =
                load("/assets/sounds/02. Start Music.mp3");

        wakaPlayer =
                load("/assets/sounds/PacmanWakaWaka04.wav");

        powerPlayer =
                load("/assets/sounds/12. Ghost - Turn to Blue.mp3");

        eatGhostPlayer =
                load("/assets/sounds/13. PAC-MAN - Eating The Ghost.mp3");

        returnPlayer =
                load("/assets/sounds/14. Ghost - Return to Home.mp3");

        deathPlayer =
                load("/assets/sounds/15. Fail.mp3");

        fruitPlayer =
                load("/assets/sounds/11. PAC-MAN - Eating The Fruit.mp3");

        if (gameplayPlayer != null) {

            gameplayPlayer.setCycleCount(
        ...
        if (startPlayer != null) {
            ...
            startPlayer.setVolume(0.22);
        }

        if (wakaPlayer != null) {

            wakaPlayer.setVolume(0.08);
        }

        if (powerPlayer != null) {

            powerPlayer.setVolume(0.16);
        }

        if (eatGhostPlayer != null) {

            eatGhostPlayer.setVolume(0.18);
        }

        if (returnPlayer != null) {

            returnPlayer.setCycleCount(
                    MediaPlayer.INDEFINITE
            );
        ...
        if (deathPlayer != null) {
            ...
            deathPlayer.setVolume(0.22);
        }

        if (fruitPlayer != null) {

            fruitPlayer.setVolume(0.14);
        }
    }
    ...
    public static void playPowerPellet() {

        playOnce(powerPlayer);
    }

    public static void playGhostEaten() {

        playOnce(eatGhostPlayer);
    }
    ...
    public static void playFruitEaten() {

        playOnce(fruitPlayer);
    }
