// clase q representa al jugador, osease Pac-Man
...
public class PacMan {
    ...
    private static final double ASSIST_RANGE = GameMap.TILE_SIZE * 0.55;
    ...
    public void update(long now) {
        ...
        if (dying) {
            animateDeath(now);
            if (now - dyingStart >= dyingDuration) respawn();
            return;
        }
        ...
        // horizontal
        if (!isWall(nextX, y)) x = nextX;

        // vertical
        if (!isWall(x, nextY)) y = nextY;

        // tunel
        if (inTunnel()) {
            double maxWidth = GameMap.getCols() * GameMap.TILE_SIZE;
            if (x < -renderSize) x = maxWidth - (GameMap.TILE_SIZE * 3);
            if (x > maxWidth)    x = GameMap.TILE_SIZE * 2;
        }
        ...
        if (pelletRow >= 0 && pelletRow < GameMap.getRows() && pelletCol >= 0 && pelletCol < GameMap.getCols()) {
            ...
            if (pts > 0) {
                // solo reproducir waka si esta vivo
                if (!dying && !frozen && !gameOver) justAtePellet = true;
                addScore(pts);
            }
        }
    ...
    // asistencia movimiento
    private void tryApplyQueuedDirection(double currentSpeed) {
        if (queuedDx == 0 && queuedDy == 0) return;

        double snapX = Math.round(x / GameMap.TILE_SIZE) * GameMap.TILE_SIZE;
        double snapY = Math.round(y / GameMap.TILE_SIZE) * GameMap.TILE_SIZE;

        boolean nearX = Math.abs(x - snapX) <= ASSIST_RANGE;
        boolean nearY = Math.abs(y - snapY) <= ASSIST_RANGE;

        if (nearX && nearY) {
            double testX = snapX + queuedDx * currentSpeed;
            double testY = snapY + queuedDy * currentSpeed;
    ...
    private void animateDeath(long now) {
        if (now - lastDeathFrame > deathFrameDelay) {
            if (deathFrame < deathFrames.length - 1) deathFrame++;
            lastDeathFrame = now;
        }
    }
    ...
    public void setDirection(int newDx, int newDy) {
        if (newDx == 1)       angle = 0;
        else if (newDx == -1) angle = 180;
        else if (newDy == -1) angle = 270;
        else if (newDy == 1)  angle = 90;
    ...
    public void increaseSpeed(double factor) {
        baseSpeed = Math.min(baseSpeed * factor, 2.2);
    }
    ...
    public boolean justAtePellet()        { return justAtePellet; }
